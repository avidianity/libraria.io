...
class File extends Model
{
     ...
     * @var array
     */
    protected $fillable = ['type', 'name', 'url', 'size', 'public'];
    ...
    public static function processURL(string $url)
    {
        ...
        if ($binary === false) {
            throw new FileNotFoundException("{$url} cannot be found.");
        }

        $data['type'] = (new finfo(FILEINFO_MIME_TYPE))->buffer($binary);
        $fileName = basename($url) . '.' . self::convertMimeToExtension($data['type']);
        $data['name'] = $fileName;
        $type = explode('/', $data['type'])[0];
        $path = $type . '/' . $data['name'];
        $data['url'] = $path;
